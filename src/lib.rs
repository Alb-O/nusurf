	...
	fn run(
		...
		if ["ws", "wss"].contains(&requested_url.scheme()) {
			let timeout = timeout.map(|ref val| {
				let duration =
					Duration::from_nanos(val.as_duration().expect("Timeout should be set to duration") as u64);
			...
			if let Some((client, websocket)) = connect(
				...
				match input {
					PipelineData::Value(val, ..) => {
						...
						ws.send(message)
							.map_err(|e| LabeledError::new(format!("Failed to send WebSocket message: {e}")))?;

						log::debug!("Message sent successfully, now starting to receive");
					}
					PipelineData::ByteStream(stream, ..) => {
						let data = stream.into_bytes().map_err(|e| LabeledError::new(e.to_string()))?;
						...
						let message = match String::from_utf8(data.clone()) {
							...
							Err(_) => tungstenite::Message::Binary(data.into()),
						};

						ws.send(message)
							.map_err(|e| LabeledError::new(format!("Failed to send WebSocket message: {e}")))?;

						log::debug!("ByteStream message sent successfully, now starting to receive");
					}
					PipelineData::Empty => {
