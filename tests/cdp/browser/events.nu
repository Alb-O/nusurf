...
def main [http_port: int, fixture_port: int] {
    ...
    try {
        ...
        try {
            ...
            let solo_event = (
                cdp event $page.session "Runtime.consoleAPICalled" --max-time 2sec
            )
            assert equal ($solo_event.params.args | get 0.value) "solo-event"

            let duplicate = (cdp event $page.session --max-time 250ms)
            assert nothing $duplicate (
                "filtered events should not remain available via unfiltered reads"
            )
            ...
            let console_begin = (
                cdp event $page.session "Runtime.consoleAPICalled" --max-time 5sec
            )
            ...
            let request_urls = (
                [$request_a.params.request.url $request_b.params.request.url]
                | str join "\n"
            )
            ...
            assert $drained_to_idle (
                "event queue should drain to idle after the routed events are consumed"
            )
        } finally {
